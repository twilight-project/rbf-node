...
func main() {
	initialize()
	validatorName := viper.GetString("accountName")
	go eventhandler.NyksEventListener("broadcast_tx_sweep", validatorName, "broadcastSweep", DbConn)
	go eventhandler.NyksEventListener("broadcast_tx_refund", validatorName, "broadcastRefund", nil)
	utils.BroadcastOnBtc(DbConn)
